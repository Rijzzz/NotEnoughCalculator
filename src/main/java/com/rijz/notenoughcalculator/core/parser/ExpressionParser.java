...
public class ExpressionParser {
	...
	public ExpressionParser(MathContext mc, Map<String, BigDecimal> variables,
			BiConsumer<String, BigDecimal> variableSetter) {
		this.mc = mc;
			...
			if (pos + 1 < tokens.size() && tokens.get(pos + 1).kind == TokenKind.ASSIGN) {
				if (ExpressionEvaluator.isReservedVariable(cleanName)) {
					throw new EvalException(
							ExpressionEvaluator.tr("notenoughcalculator.error.reserved_variable", cleanName),
							tokens.get(pos).pos);
				}
	...
	private ParseResult parseBitwiseOr(List<Token> tokens, int pos) throws EvalException {
		...
		while (currentPos < tokens.size() && tokens.get(currentPos).kind == TokenKind.OP
				&& tokens.get(currentPos).value.equals("|")) {
			ParseResult right = parseBitwiseAnd(tokens, currentPos + 1);
			if (right.radixMode != RadixMode.NONE)
				mode = right.radixMode;
			try {
	...
	// Addition and subtraction with smart percentage calculation (e.g., 100 + 10%
	// -> 100 + (100 * 0.10) = 110)
	private ParseResult parseAddSub(List<Token> tokens, int pos) throws EvalException {
		...
		while (currentPos < tokens.size() && tokens.get(currentPos).kind == TokenKind.OP
			...
			if (right.radixMode != RadixMode.NONE && right.radixMode != RadixMode.DEFAULT)
				mode = right.radixMode;
		...
		while (currentPos < tokens.size() && tokens.get(currentPos).kind == TokenKind.OP
			...
			if (op.equals("*") || op.equals("x")) {
				...
				if (right.value.compareTo(BigDecimal.ZERO) == 0) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.division_by_zero"),
							currentPos);
				}
				res = left.value.divide(right.value, mc);
			} else {
				if (right.value.compareTo(BigDecimal.ZERO) == 0) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.modulo_by_zero"),
							currentPos);
				}
	...
	private ParseResult parsePower(List<Token> tokens, int pos) throws EvalException {
		...
		if (currentPos < tokens.size() && tokens.get(currentPos).kind == TokenKind.OP
				&& tokens.get(currentPos).value.equals("^")) {
			ParseResult right = parsePower(tokens, currentPos + 1);
			...
			try {
				...
				if (exp < -1000 || exp > 1000) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.exponent_too_large"),
							currentPos);
				}
				BigDecimal res = left.value.pow(exp, mc);
				return new ParseResult(res, right.nextPos,
						left.radixMode != RadixMode.NONE ? left.radixMode : right.radixMode);
			} catch (ArithmeticException e) {
				if (left.value.compareTo(BigDecimal.ZERO) < 0) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.negative_power"),
							currentPos);
				}
				double baseD = left.value.doubleValue();
				double expD = right.value.doubleValue();
				double resD = Math.pow(baseD, expD);
				return new ParseResult(BigDecimal.valueOf(resD), right.nextPos,
						left.radixMode != RadixMode.NONE ? left.radixMode : right.radixMode);
			}
		}
	...
	private ParseResult parseFactorial(List<Token> tokens, int pos) throws EvalException {
		...
		while (currentPos < tokens.size() && tokens.get(currentPos).kind == TokenKind.FACTORIAL) {
			...
			try {
				...
				if (n < 0) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.factorial_non_integer"),
							currentPos);
				}
				if (n > 1000) {
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.factorial_too_large"),
							currentPos);
				}
	...
	private ParseResult parsePrimary(List<Token> tokens, int pos) throws EvalException {
		...
		if (tok.kind == TokenKind.NUM) {
			...
			if (tok.value.startsWith("0b") || tok.value.startsWith("0B"))
				mode = RadixMode.BIN;
			else if (tok.value.startsWith("0x") || tok.value.startsWith("0X"))
				mode = RadixMode.HEX;
			else if (tok.value.startsWith("0o") || tok.value.startsWith("0O"))
				mode = RadixMode.OCT;
		...
		if (tok.kind == TokenKind.VAR) {
			if (!tok.value.startsWith("$")) {
				throw new EvalException(
						ExpressionEvaluator.tr("notenoughcalculator.error.undefined_variable", tok.value), tok.pos);
			}
			...
			if (ExpressionEvaluator.BUILTIN_VARIABLES.contains(cleanName)
					|| ExpressionEvaluator.BUILTIN_VARIABLES.contains(rawName)) {
				throw new EvalException(
						ExpressionEvaluator.tr("notenoughcalculator.error.api_data_unavailable", cleanName), tok.pos);
			}

			throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.undefined_variable", tok.value),
					tok.pos);
		}
		...
		if (tok.kind == TokenKind.FUNC) {
			if (pos + 1 >= tokens.size() || tokens.get(pos + 1).kind != TokenKind.LPAREN) {
				throw new EvalException(
						ExpressionEvaluator.tr("notenoughcalculator.error.expected_parenthesis", tok.value), tok.pos);
			}
			...
			if (isMarketFunction(fnName)) {
				...
				if (nextPos < tokens.size() && tokens.get(nextPos).kind == TokenKind.VAR) {
					...
					if (endPos < tokens.size() && tokens.get(endPos).kind == TokenKind.RPAREN) {
						...
						throw new EvalException(
								ExpressionEvaluator.tr("notenoughcalculator.error.item_not_found", itemId), tok.pos);
					}
				}
				throw new EvalException(
						ExpressionEvaluator.tr("notenoughcalculator.error.invalid_market_syntax", fnName), tok.pos);
			}

			if (fnName.equals("perk") || fnName.equals("hotmperk") || fnName.equals("hperk")) {
				int nextPos = pos + 2;
				if (nextPos < tokens.size()
						&& (tokens.get(nextPos).kind == TokenKind.VAR || tokens.get(nextPos).kind == TokenKind.NUM)) {
			...
			if (fnName.equals("skillxp") || fnName.equals("skill_xp") || fnName.equals("skilltable")) {
				...
				if (nextPos < tokens.size() && tokens.get(nextPos).kind == TokenKind.COMMA) {
					...
					if (nextPos >= tokens.size() || tokens.get(nextPos).kind != TokenKind.RPAREN) {
						throw new EvalException(
								ExpressionEvaluator.tr("notenoughcalculator.error.expected_closing_paren"), nextPos);
					}
					BigDecimal xp = SkillXpTable.getSkillXpBetween(arg1.value.intValue(), arg2.value.intValue());
					return new ParseResult(xp, nextPos + 1,
							arg1.radixMode != RadixMode.NONE ? arg1.radixMode : arg2.radixMode);
				...
				throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.expected_closing_paren"),
						nextPos);
			}
			...
			if (fnName.equals("cataxp") || fnName.equals("cata_xp") || fnName.equals("catatable")
					|| fnName.equals("cxp_table")) {
				ParseResult arg1 = parseExpression(tokens, pos + 2);
				int nextPos = arg1.nextPos;
				if (nextPos < tokens.size() && tokens.get(nextPos).kind == TokenKind.COMMA) {
					ParseResult arg2 = parseExpression(tokens, nextPos + 1);
			...
			if (isSlayerFunction(fnName)) {
				...
				if ((firstTok.kind == TokenKind.VAR || firstTok.kind == TokenKind.FUNC) && curPos + 1 < tokens.size()
						&& tokens.get(curPos + 1).kind == TokenKind.COMMA) {
					boss = firstTok.value;
					curPos += 2;
				}
				...
				if (nextPos < tokens.size() && tokens.get(nextPos).kind == TokenKind.COMMA) {
					...
					BigDecimal xp = SlayerXpTable.getSlayerXpBetween(boss, arg1.value.intValue(),
							arg2.value.intValue());
					return new ParseResult(xp, nextPos + 1,
							arg1.radixMode != RadixMode.NONE ? arg1.radixMode : arg2.radixMode);
				} else if (nextPos < tokens.size() && tokens.get(nextPos).kind == TokenKind.RPAREN) {
			...
			if (fnName.equals("avg")) {
				...
				while (curPos < tokens.size() && tokens.get(curPos).kind != TokenKind.RPAREN) {
					...
					if (argRes.radixMode != RadixMode.NONE)
						mode = argRes.radixMode;
			...
			if (fnName.equals("min") || fnName.equals("max") || fnName.equals("pct") || fnName.equals("gcd")
					|| fnName.equals("lcm") || fnName.equals("xor") || fnName.equals("ahbin")) {
				ParseResult arg1 = parseExpression(tokens, pos + 2);
				int nextPos = arg1.nextPos;
				...
				if (nextPos >= tokens.size() || tokens.get(nextPos).kind != TokenKind.COMMA) {
					...
					throw new EvalException(
							ExpressionEvaluator.tr("notenoughcalculator.error.expected_comma", tok.value), nextPos);
				}
				...
				return new ParseResult(resultVal, nextPos + 1,
						arg1.radixMode != RadixMode.NONE ? arg1.radixMode : arg2.radixMode);
			}
			...
			if (fnName.equals("clamp")) {
				...
				int np = vRes.nextPos;
				if (np >= tokens.size() || tokens.get(np).kind != TokenKind.COMMA)
					throw new EvalException(
							ExpressionEvaluator.tr("notenoughcalculator.error.expected_comma", tok.value), np);
				...
				np = maxRes.nextPos;
				if (np >= tokens.size() || tokens.get(np).kind != TokenKind.RPAREN)
					throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.expected_closing_paren"),
							np);
			...
			if (fnName.equals("hex"))
				mode = RadixMode.HEX;
			else if (fnName.equals("bin"))
				mode = RadixMode.BIN;
			else if (fnName.equals("oct"))
				mode = RadixMode.OCT;
			else if (fnName.equals("fmt"))
				mode = RadixMode.SHORTHAND;
		...
		throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.unexpected_token", tok.value),
				tok.pos);
	}
	...
	private BigDecimal applyBinaryFunction(String func, BigDecimal a, BigDecimal b, int pos) throws EvalException {
		switch (func) {
			...
			default :
				throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.unknown_function", func),
						pos);
		}
	}
	...
	private static boolean isSlayerFunction(String fn) {
		return fn.equals("slayerxp") || fn.equals("slayer_xp") || fn.equals("slayertable") || fn.equals("zombiexp")
				|| fn.equals("wolfxp") || fn.equals("svenxp") || fn.equals("revxp") || fn.equals("revenantxp")
				|| fn.equals("spiderxp") || fn.equals("spider_xp") || fn.equals("tarantulaxp")
				|| fn.equals("tarantula_xp") || fn.equals("spidertable") || fn.equals("emanxp")
				|| fn.equals("voidgloomxp") || fn.equals("blazexp") || fn.equals("infernoxp") || fn.equals("endermanxp")
