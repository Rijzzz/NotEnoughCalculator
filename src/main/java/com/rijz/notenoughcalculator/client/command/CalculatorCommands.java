...
public class CalculatorCommands {
	...
	public static int executeCalc(CommandContext<FabricClientCommandSource> ctx) {
		...
		try {
			...
			String copyText = config.enableFullEquationCopy
					? ResultFormatter.formatEquationForCopy(expr, formatted)
					: formatted;
			String highlightedExpr = config.enableSyntaxHighlighting
					? SyntaxHighlighter.highlight(expr)
					: SyntaxHighlighter.getColorNumber() + expr;

			MutableComponent msg = Component.literal(prefix + highlightedExpr + ResultFormatter.getEqualsSign()
					+ config.getChatResultColorCode() + formatted);

			Component tooltip = t("notenoughcalculator.chat.click_to_copy_tooltip");
			msg.setStyle(msg.getStyle().withClickEvent(new ClickEvent.RunCommand("/calccopy " + copyText))
	...
	public static int executeSet(CommandContext<FabricClientCommandSource> ctx) {
		...
		try {
			...
			send(ctx, "notenoughcalculator.variable.set", varName, ResultFormatter.getEqualsSign(),
					config.getChatResultColorCode() + formatted);
		} catch (EvalException e) {
			sendLiteral(ctx, config.getErrorColorCode() + t("notenoughcalculator.result.error_prefix").getString()
					+ e.getMessage());
		}
