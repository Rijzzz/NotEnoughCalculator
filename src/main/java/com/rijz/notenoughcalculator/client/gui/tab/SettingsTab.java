...
public class SettingsTab {
	...
	public void init(CalculatorConfigScreen screen, int panelX, int startY, int buttonWidth, int buttonHeight,
			Runnable reinitAction) {
		...
		syntaxBtn = screen.addRenderableWidgetPublic(Button.builder(Component.empty(), btn -> {
			...
			screen.updateButtonLabelsPublic();
				...
				.tooltip(Tooltip
						.create(Component.translatable("notenoughcalculator.config.tooltip.syntax_highlighting")))
				.build());
		...
		Tooltip bazaarTooltip = Tooltip
				.create(Component.translatable("notenoughcalculator.config.tooltip.bazaar_flipper_perk"));

		bazaarLvl0Btn = screen.addRenderableWidgetPublic(Button
				.builder(Component.translatable("notenoughcalculator.config.screen.bazaar_flipper_lvl_0"),
						btn -> screen.selectBazaarLevelPublic(0))
				.bounds(pillsStartX, pillsY, pillWidth, buttonHeight).tooltip(bazaarTooltip).build());

		bazaarLvl1Btn = screen.addRenderableWidgetPublic(Button
				.builder(Component.translatable("notenoughcalculator.config.screen.bazaar_flipper_lvl_1"),
						btn -> screen.selectBazaarLevelPublic(1))
				.bounds(pillsStartX + pillWidth + pillGap, pillsY, pillWidth, buttonHeight).tooltip(bazaarTooltip)
				.build());

		bazaarLvl2Btn = screen.addRenderableWidgetPublic(Button
				.builder(Component.translatable("notenoughcalculator.config.screen.bazaar_flipper_lvl_2"),
						btn -> screen.selectBazaarLevelPublic(2))
				.bounds(pillsStartX + (pillWidth + pillGap) * 2, pillsY, pillWidth, buttonHeight).tooltip(bazaarTooltip)
				.build());
		...
		historyBtn = screen.addRenderableWidgetPublic(Button.builder(Component.empty(), btn -> {
			...
			screen.updateButtonLabelsPublic();
				...
				.tooltip(
						Tooltip.create(Component.translatable("notenoughcalculator.config.tooltip.history_navigation")))
				.build());
		currY += 18;
		...
		fullCopyBtn = screen.addRenderableWidgetPublic(Button.builder(Component.empty(), btn -> {
			...
			screen.updateButtonLabelsPublic();
				...
				.tooltip(
						Tooltip.create(Component.translatable("notenoughcalculator.config.tooltip.full_equation_copy")))
				.build());
		currY += 18;
		...
		if (hasOtherIntegrations) {
			standaloneBtn = screen.addRenderableWidgetPublic(Button.builder(Component.empty(), btn -> {
				...
				if (reinitAction != null) {
					...
					.tooltip(Tooltip
							.create(Component.translatable("notenoughcalculator.config.tooltip.force_standalone_mode")))
					.build());
			currY += 18;
		} else {
			standaloneBtn = null;
		...
		if (!hasOtherIntegrations || screen.forceStandaloneMode) {
			...
			screen.addRenderableWidgetPublic(
					Button.builder(Component.translatable("notenoughcalculator.config.screen.edit_gui"), btn -> {
						ReflectionUtils.openScreen(screen.getMinecraftInstancePublic(),
								new PositionConfigScreen(screen));
					}).bounds(panelX + 20, currY, editWidth, buttonHeight)
							.tooltip(Tooltip
									.create(Component.translatable("notenoughcalculator.config.tooltip.edit_gui")))
							.build());

			screen.addRenderableWidgetPublic(
					Button.builder(Component.translatable("notenoughcalculator.config.screen.reset_symbol"), btn -> {
						...
						screen.updateButtonLabelsPublic();
							...
							.tooltip(Tooltip.create(
									Component.translatable("notenoughcalculator.config.tooltip.reset_position")))
							.build());
			currY += 18;
		}
		...
		if (IntegrationManager.isItemListLoaded()) {
			itemListBtn = screen.addRenderableWidgetPublic(Button.builder(Component.empty(), btn -> {
				...
				screen.updateButtonLabelsPublic();
					...
					.tooltip(Tooltip
							.create(Component.translatable("notenoughcalculator.config.tooltip.itemlist_integration")))
					.build());
		} else {
			itemListBtn = null;
		}
	...
	public void updateLabels(CalculatorConfigScreen screen) {
		...
		if (precisionBtn != null) {
			precisionBtn.setMessage(Component.translatable("notenoughcalculator.config.screen.decimal_precision").copy()
					.append(Component.translatable("notenoughcalculator.config.screen.decimal_precision_format",
							String.valueOf(screen.decimalPrecision))));
		}
		if (historyBtn != null) {
	...
	public void renderSectionHeaders(GuiGraphicsExtractor graphics, Font font, int panelX, int startY) {
		graphics.text(font, Component.translatable("notenoughcalculator.config.screen.display_header"), panelX + 20,
				startY + 2, ColorConstants.TEXT_EMERALD, true);
		graphics.text(font, Component.translatable("notenoughcalculator.config.screen.calculation_header"), panelX + 20,
				startY + 106, ColorConstants.TEXT_SKY, true);
		graphics.text(font, Component.translatable("notenoughcalculator.config.screen.bazaar_flipper_level_label"),
				panelX + 20, startY + 138, ColorConstants.TEXT_SLATE, true);
		graphics.text(font, Component.translatable("notenoughcalculator.config.screen.features_header"), panelX + 20,
				startY + 170, ColorConstants.TEXT_AMBER, true);
	}
}
