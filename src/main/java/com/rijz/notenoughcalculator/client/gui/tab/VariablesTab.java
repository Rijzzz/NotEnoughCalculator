...
public class VariablesTab {
	...
	public void init(CalculatorConfigScreen screen, Font font, int panelX, int startY, Runnable reinitAction) {
		...
		varNameInput = new EditBox(font, panelX + 20, inputY, 110, 18,
				Component.translatable("notenoughcalculator.config.var.name_placeholder"));
		...
		varValueInput = new EditBox(font, panelX + 136, inputY, 118, 18,
				Component.translatable("notenoughcalculator.config.var.value_placeholder"));
		...
		screen.addRenderableWidgetPublic(
				Button.builder(Component.translatable("notenoughcalculator.config.var.add_button"), btn -> {
					...
					if (cleanName.isEmpty() || value.isEmpty()) {
						varStatusMessage = Component.translatable("notenoughcalculator.config.var.error.enter_name_val")
								.getString();
					} else if (!cleanName.matches("^[a-zA-Z0-9_]+$")) {
						varStatusMessage = Component
								.translatable("notenoughcalculator.config.var.error.invalid_name", cleanName)
								.getString();
					} else if (ExpressionEvaluator.isReservedVariable(cleanName)) {
						varStatusMessage = Component
								.translatable("notenoughcalculator.config.var.error.reserved", cleanName).getString();
					} else {
						screen.workingCustomVariables.put(cleanName, value);
						varNameInput.setValue("");
						varValueInput.setValue("");
						varStatusMessage = Component
								.translatable("notenoughcalculator.config.var.success_set", cleanName, value)
								.getString();
						reinitAction.run();
					}
		...
		if (screen.varPage >= totalPages)
			screen.varPage = totalPages - 1;
		if (screen.varPage < 0)
			screen.varPage = 0;
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			Button removeBtn = screen.addRenderableWidgetPublic(
					Button.builder(Component.translatable("notenoughcalculator.config.var.remove_button"), btn -> {
						screen.workingCustomVariables.remove(varKey);
						reinitAction.run();
					}).bounds(panelX + 265, rowY, 35, 16)
							.tooltip(Tooltip
									.create(Component.translatable("notenoughcalculator.config.tooltip.remove_var")))
							.build());
		...
		if (totalPages > 1) {
			prevPageBtn = screen.addRenderableWidgetPublic(
					Button.builder(Component.translatable("notenoughcalculator.config.var.prev_page"), btn -> {
						if (screen.varPage > 0) {
							screen.varPage--;
							reinitAction.run();
						}
			...
			nextPageBtn = screen.addRenderableWidgetPublic(
					Button.builder(Component.translatable("notenoughcalculator.config.var.next_page"), btn -> {
	...
	public void renderContent(GuiGraphicsExtractor graphics, Font font, CalculatorConfigScreen screen, int panelX,
			int startY) {
		graphics.text(font, Component.translatable("notenoughcalculator.config.var.section_title"), panelX + 20,
				startY + 2, ColorConstants.TEXT_SKY, true);
		if (varStatusMessage != null && !varStatusMessage.isEmpty()) {
			graphics.text(font, Component.literal(varStatusMessage), panelX + 20, startY + 37,
					ColorConstants.TEXT_WHITE, false);
		}
		...
		for (int i = startIndex; i < endIndex; i++) {
			String keyStr = Component
					.translatable("notenoughcalculator.config.var.key_format", varEntries.get(i).getKey()).getString();
			String valStr = Component
					.translatable("notenoughcalculator.config.var.val_format", varEntries.get(i).getValue())
					.getString();
		...
		if (screen.workingCustomVariables.isEmpty()) {
			graphics.text(font, Component.translatable("notenoughcalculator.config.var.empty_notice"), panelX + 20,
					startY + 52, ColorConstants.TEXT_MUTED, true);
		} else if (totalPages > 1) {
			Component pageComp = Component.translatable("notenoughcalculator.config.var.page_format",
					(screen.varPage + 1), totalPages);
