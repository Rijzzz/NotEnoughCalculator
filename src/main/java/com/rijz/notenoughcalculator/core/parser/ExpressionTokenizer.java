		...
		while (i < expr.length()) {
				...
				if (radix > 0) {
					...
					if (!rawVal.isEmpty()) {
						try {
							...
							throw new EvalException(ExpressionEvaluator.tr("notenoughcalculator.error.invalid_number"),
									start);
						}
					}
			...
			if (c == '$' || Character.isLetter(c) || c == '_') {
				...
				if (ExpressionEvaluator.FUNCTIONS.contains(nameStr)) {
					...
					if (!nameStr.startsWith("$") && !tokens.isEmpty()
							&& tokens.get(tokens.size() - 1).kind == TokenKind.NUM) {
					...
					tokens.add(new Token(TokenKind.NUM, nameStr, start,
							lastAnswer != null ? lastAnswer : BigDecimal.ZERO));
				} else {
					tokens.add(new Token(TokenKind.VAR, nameStr, start));
				}
				continue;
	...
	private static List<Token> insertImplicitMultiplication(List<Token> tokens) {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (i + 1 < tokens.size()) {
				...
				if (cur.kind == TokenKind.NUM && next.kind == TokenKind.LPAREN)
					insert = true;
				if (cur.kind == TokenKind.NUM && next.kind == TokenKind.FUNC)
					insert = true;
				if (cur.kind == TokenKind.RPAREN && next.kind == TokenKind.NUM)
					insert = true;
				if (cur.kind == TokenKind.RPAREN && next.kind == TokenKind.LPAREN)
					insert = true;
				if (cur.kind == TokenKind.RPAREN && next.kind == TokenKind.FUNC)
					insert = true;
				if (cur.kind == TokenKind.UNIT && next.kind == TokenKind.LPAREN)
					insert = true;
				if (cur.kind == TokenKind.PERCENT
						&& (next.kind == TokenKind.NUM || next.kind == TokenKind.LPAREN || next.kind == TokenKind.FUNC))
					insert = true;
