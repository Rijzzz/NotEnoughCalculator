...
public class NotEnoughCalculatorClient implements ClientModInitializer {
				...
				if (runtime != null) {
					...
					if (searchField != null) {
						...
						if (key == GLFW.GLFW_KEY_C && isCtrlOrCmd && isCalculation && hasResult
							...
							String result = calcManager.getLastFormattedResult();
							if (result != null && !result.isEmpty()) {
								String copyText = enableFullCopy
										? ResultFormatter.formatEquationForCopy(searchText, result)
										: result;
	...
	private void registerCommands() {
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
			...
			dispatcher.register(ClientCommands.literal("calchist").executes(CalculatorCommands::executeHistory));
			dispatcher.register(ClientCommands.literal("calcclear").executes(CalculatorCommands::executeClear));
			...
			dispatcher.register(ClientCommands.literal("calchelp").executes(CalculatorCommands::executeHelp)
					.then(ClientCommands.argument("page", StringArgumentType.word()).suggests((ctx,
							builder) -> SharedSuggestionProvider.suggest(new String[]{"main", "operators", "functions",
									"units", "variables", "stats", "market", "tax", "examples", "config"}, builder))
							.executes(CalculatorCommands::executeHelpPage)));

			dispatcher.register(ClientCommands.literal("calcconfig").executes(CalculatorCommands::executeConfig)
					.then(ClientCommands.literal("position").executes(CalculatorCommands::executePosition)));

			dispatcher.register(ClientCommands.literal("calcpos").executes(CalculatorCommands::executePosition));

			dispatcher.register(ClientCommands.literal("calccopy").then(ClientCommands
					.argument("text", StringArgumentType.greedyString()).executes(CalculatorCommands::executeCopy)));
		});
	}
